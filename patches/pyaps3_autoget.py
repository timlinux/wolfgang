# This program patches pyaps3 so it will download ERA5 data concurrently
...
def ECMWFdload(bdate, hr, filedir, model='ERA5', datatype='fc', humidity='Q', snwe=None, flist=None):
    ...
    def download_weather_data(i, day):
        fname = flist[i]
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        executor.map(
            ...
            [i for i in range(len(bdate))],
            [day for day in bdate]
        )
    return flist
...
def MERRAdload(bdate, hr, filedir, hdf=False):
    ...
    for i, date in enumerate(bdate):
        ...
        if year < 1992:
            weburl = '%s%s%s%s%s%s%s%s%s%s%s%s%s' % (
            url1, yr, url2, mon, url3o, date, url4, hr, url5, hr, url6o, date, url7)
        elif year < 2001:
            weburl = '%s%s%s%s%s%s%s%s%s%s%s%s%s' % (
            url1, yr, url2, mon, url3, date, url4, hr, url5, hr, url6, date, url7)
        else:
            weburl = '%s%s%s%s%s%s%s%s%s%s%s%s%s' % (
            url1, yr, url2, mon, url3n, date, url4, hr, url5, hr, url6n, date, url7)
        dir = '%s' % (filename)
